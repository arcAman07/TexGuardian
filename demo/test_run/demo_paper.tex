...
% DELIBERATE BUG: Title is oversized
\title{\fontsize{30}{34}\selectfont Scaling Sparse Mixture-of-Experts for\\[16pt]Long-Context Document Understanding}
\author{
...
% DELIBERATE BUG: Abstract heading wrong — uses \section* instead of \begin{abstract}
...
\vspace{24pt}  % DELIBERATE BUG: Excessive space after abstract

\section{Introduction}

Long-context document understanding remains challenging \cite{devlin2019bert}.
Modern documents contain heterogeneous content that requires different strategies
\cite{vaswani2017attention, brown2020language}. MoE architectures
\cite{shazeer2017outrageously} offer conditional computation but existing
approaches do not account for document structure \cite{nonexistent_paper_2024}.
Recent retrieval-augmented methods \cite{hallucinated_reference_2023} and
structure-aware routing \cite{also_fake_citation_2025} motivate our approach.
% FIXME: Need to strengthen this motivation paragraph
Our contributions: (1) structure-aware routing, (2) sub-quadratic attention
scaling to 128K tokens, (3) evaluation on four benchmarks (Figure~\ref{fig:nonexistent_figure}).

% DELIBERATE BUG: Table overflows RIGHT margin — 9 columns with long headers
\begin{table}[H]
    \caption{Comparison of document understanding approaches. $\checkmark$ = supported, $\times$ = not supported.}
...
% DELIBERATE BUG: Architecture diagram overflows — nodes spread past margins
\begin{figure}[H]
    \noindent
    \hspace{-1cm}%
...
types $c_i \in \{$prose, table, equation, caption$\}$. The routing function is:
$g(s_i) = \text{TopK}(\text{softmax}(W_r \cdot \text{pool}(s_i) + b_r), k)$.
% TODO: Add the load balancing loss equation here

% DELIBERATE BUG: Expert specialization chart — overflows RIGHT (1.5x width)
\begin{figure}[H]
    \noindent
    \begin{tikzpicture}
    \begin{axis}[
...
% DELIBERATE BUG: Results table overflows RIGHT margin (too many columns + \hline)
\begin{table}[H]
...
% DELIBERATE BUG: Table with \hline instead of booktabs
\begin{table}[H]
...
\cite{zhang2019hibert, xu2020layoutlm, beltagy2020longformer, fedus2022switch,
lepikhin2020gshard, radford2019language, talmor2019multiqa, another_missing_ref}.
