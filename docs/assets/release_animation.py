"""TexGuardian release animation — ~55s high-definition demo.

Based on real CLI output. Shows /venue + /review full pipeline.

Render (1080p):
  manim -qh release_animation.py TexGuardianRelease

Convert to GIF:
  ffmpeg -y -i <mp4> -vf "fps=15,scale=1280:-1:flags=lanczos,split[s0][s1]; \
    [s0]palettegen=max_colors=256[p];[s1][p]paletteuse=dither=floyd_steinberg" \
    -loop 0 release_animation.gif
"""

from manim import *

# ── Bright palette (Tailwind 400) for dark backgrounds ──
BLUE = "#3B82F6"
GREEN = "#22C55E"
GRAY = "#94A3B8"
LIGHT = "#E2E8F0"
...
class TexGuardianRelease(Scene):
    def construct(self):
        ...
        # ═══════════════════════════════════════════════
        shield = self._shield().scale(1.3).move_to(ORIGIN)
        title = Text("TexGuardian", font=SANS, weight=BOLD, font_size=62)
        title[:3].set_color(WHITE)
        title[3:].set_color(BLUE)
        title.next_to(shield, RIGHT, buff=0.5)
        logo = VGroup(shield, title).move_to(ORIGIN)

        self.play(FadeIn(shield, scale=0.3), run_time=0.7)
        self.play(Write(title), run_time=0.9)

        tagline = Text(
            "AI-powered LaTeX paper assistant",
            font=SANS, font_size=26, color=GRAY,
        )
        tagline.next_to(logo, DOWN, buff=0.45)
        self.play(FadeIn(tagline, shift=UP * 0.2), run_time=0.5)
        self.wait(0.8)

        # ═══════════════════════════════════════════════
        # Phase 2 — Terminal + welcome panel (3–7s)
        # ═══════════════════════════════════════════════
        self.play(
            logo.animate.scale(0.42).to_edge(UP, buff=0.2),
            FadeOut(tagline),
            run_time=0.5,
        )
        ...
        term_bg = RoundedRectangle(
            corner_radius=0.15, width=12.8, height=6.3,
            fill_color=TERM_BG, fill_opacity=1,
            stroke_color=TERM_BORDER, stroke_width=2,
        )
        term_bar = Rectangle(
            width=12.8, height=0.38,
            fill_color=TERM_BAR_BG, fill_opacity=1, stroke_width=0,
        )
        term_bar.next_to(term_bg, UP, buff=0).shift(DOWN * 0.19)
        dots = VGroup(
        ...
        venue_cmd = self._row([
            ("❯ ", BLUE),
            ("/venue", PURPLE),
            (" let's use iclr template", LIGHT),
        ])
        venue_cmd.move_to(anchor, aligned_edge=LEFT)
        self.play(FadeIn(venue_cmd), run_time=0.3)
        self.wait(0.3)

        y = anchor[1] - LH * 1.8
        venue_grp = VGroup(venue_cmd)

        venue_lines = [
            ("  Searching GitHub: ICLR/Master-Template...", GRAY, 0.5),
            ("    ✓ Downloaded: iclr2026_conference.sty", GREEN, 0.2),
            ("    ✓ Downloaded: iclr2026_conference.bst", GREEN, 0.2),
            ("    ✓ Downloaded: natbib.sty", GREEN, 0.2),
            ("    ✓ Downloaded: fancyhdr.sty", GREEN, 0.2),
            ("", None, 0.1),
            ("  ✓ Template files downloaded!", GREEN, 0.4),
            ("  ✓ Auto-updated main .tex file", GREEN, 0.4),
        ]

        for text, color, pause in venue_lines:
            if not text:
                y -= LH * 0.5
                continue
            line = Text(text, font=MONO, font_size=FS, color=color)
            line.move_to([anchor[0], y, 0], aligned_edge=LEFT)
            y -= LH
            self.play(FadeIn(line, shift=RIGHT * 0.06), run_time=0.15)
            venue_grp.add(line)
            self.wait(pause)
        ...
        # ═══════════════════════════════════════════════
        # Phase 4 — /review full + header (12.5–16s)
        # ═══════════════════════════════════════════════
        self.play(FadeOut(venue_grp), run_time=0.3)

        review_cmd = self._row([
            ("❯ ", BLUE),
            ("/review full", PURPLE),
            (" the entire paper before submission", GRAY),
        ])
        review_cmd.move_to(anchor, aligned_edge=LEFT)
        self.play(FadeIn(review_cmd), run_time=0.3)
        self.wait(0.3)
        ...
        rh.next_to(review_cmd, DOWN, buff=0.3, aligned_edge=LEFT)
        self.play(FadeIn(rh, shift=UP * 0.1), run_time=0.4)
        self.wait(1.5)

        # ═══════════════════════════════════════════════
        # Phase 5 — Steps 1–7 ticking off (16–34s)
        # ═══════════════════════════════════════════════
        self.play(FadeOut(review_cmd), FadeOut(rh), run_time=0.3)

        round_hdr = Text(
            "── Round 1/5 " + "─" * 33,
            font=MONO, font_size=FS_SM, color=GRAY,
        )
        round_hdr.move_to(anchor, aligned_edge=LEFT)
        self.play(FadeIn(round_hdr), run_time=0.2)

        y = anchor[1] - LH * 1.5
        phase5 = VGroup(round_hdr)

        steps = [
            ("Step 1/7: Compiling LaTeX",
             "  ✓ Compiled successfully · Pages: 4", GREEN, 1.0),
            ("Step 2/7: Running Verification Checks",
             "  Found 3 issue(s): citation, overflow, hline",
             ORANGE, 1.3),
        ...
        for name, result, color, pause in steps:
            sep = Line(
                [anchor[0], y + 0.06, 0],
                [anchor[0] + 11.8, y + 0.06, 0],
                color=TERM_BORDER, stroke_width=0.8,
            )
            y -= 0.06

            hdr = Text(
                name, font=MONO, font_size=FS,
        ...
        score = self._row([
            ("  Overall Score: ", WHITE_T), ("98/100", GREEN),
        ])
        ...
        phase5.add(sep_f, score, ready)
        self.wait(2.0)

        # ═══════════════════════════════════════════════
        # Phase 6 — Summary table (34–40s)
        # ═══════════════════════════════════════════════
        self.play(FadeOut(phase5), run_time=0.4)
        ...
        # ═══════════════════════════════════════════════
        # Phase 8 — CTA: pip install + GitHub (46–55s)
        # ═══════════════════════════════════════════════
        ...
        cmd_bg = RoundedRectangle(
            corner_radius=0.15,
            width=cmd.width + 0.8, height=cmd.height + 0.5,
    ...
    def _row(self, parts):
        """Colored text row from (text, color) tuples."""
        texts = [Text(t, font=MONO, font_size=FS, color=c) for t, c in parts]
        return VGroup(*texts).arrange(RIGHT, buff=0.06)

    def _tbl_row(self, col1, col2, col3, text_color, status_color):
        """Fixed-width table row with proper column alignment."""
        ...
        return VGroup(c1, c2, c3).arrange(RIGHT, buff=0.2)

    def _panel(self, lines_data, border_color=GRAY):
        """Rich-style bordered panel. None = empty-line spacer."""
        rows = VGroup()
        ...
        border = RoundedRectangle(
            corner_radius=0.12,
            width=rows.width + 0.7, height=rows.height + 0.55,
    ...
    def _summary_table(self):
        ...
        tbl.add(Text(
            "─" * 48, font=MONO, font_size=FS - 2, color=TERM_BORDER,
        ))
        ...
        tbl.arrange(DOWN, buff=0.08, aligned_edge=LEFT)

        # Score box
        sc = self._row([("Overall Score: ", WHITE_T), ("98/100", GREEN)])
        sl = Text(
            "Excellent - Ready for submission",
            font=MONO, font_size=FS_SM, color=GREEN,
        )
        sc_content = VGroup(sc, sl).arrange(DOWN, buff=0.15)
        sc_border = RoundedRectangle(
            corner_radius=0.1,
            width=sc_content.width + 0.6, height=sc_content.height + 0.35,
            stroke_color=GREEN, stroke_width=2, fill_opacity=0,
        )
        sc_border.move_to(sc_content)
        sc_box = VGroup(sc_border, sc_content)

        return VGroup(title, tbl, sc_box).arrange(DOWN, buff=0.35)
    ...
    def _shield(self):
        ...
        body = RoundedRectangle(
            ...
            stroke_color=BLUE, stroke_width=3.5,
        )
        point = Polygon(
